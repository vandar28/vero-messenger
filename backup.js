...
class DatabaseBackup {
  constructor() {
    ...
    this.minBackupInterval = 5000; // 5 секунд между бэкапами
  ...
  validateBackup(filePath) {
    try {
      ...
      if (compressed.length < 50) return false;
      
      // Пытаемся распаковать
      const data = zlib.gunzipSync(compressed);
      
      // Проверяем сигнатуру SQLite (более гибкая проверка)
      const header = data.slice(0, 16).toString('hex');
      // SQLite сигнатура: "SQLite format 3\0"
      if (!header.startsWith('53514c69746520666f726d6174')) {
        // Возможно это другая версия SQLite, проверяем наличие ключевых таблиц
        const str = data.toString('utf8', 0, Math.min(data.length, 3000));
        if (!str.includes('users') && !str.includes('CREATE TABLE')) {
          return false;
        }
      }
      
      // Проверяем наличие хоть каких-то данных
