...
app.post('/api/backup/force', auth, adminAuth, async (req, res) => {
  try {
    await backup.instantBackup('ручной');
...
app.post('/api/register', uploadAvatar.single('avatar'), async function(req, res) {
  ...
  if (!isTemp) { 
    const permCount = (dbGet('SELECT COUNT(*) as c FROM users WHERE is_temp=0') || {}).c || 0; 
  ...
  const user = dbGet('SELECT * FROM users WHERE email=?', [b.email]);
...
app.post('/api/delete-temp-account', auth, function(req, res) {
  const user = dbGet('SELECT * FROM users WHERE id=? AND is_temp=1', [req.userId]);
...
app.post('/api/user/change-username', auth, function(req, res) {
  ...
  const existing = dbGet('SELECT id FROM users WHERE username=? AND id!=?', [trimmed, req.userId]);
  if (existing) {
    return res.status(400).json({ error: 'Это имя уже занято' });
  }
  
  dbRun('UPDATE users SET username=? WHERE id=?', [trimmed, req.userId]);
  saveDB();
  
  const user = dbGet('SELECT id, username, email, avatar, is_temp FROM users WHERE id=?', [req.userId]);
...
app.get('/api/user/status/:userId', auth, function(req, res) {
  const user = dbGet('SELECT is_online, last_seen, is_typing, typing_to FROM users WHERE id=?', [req.params.userId]);
...
app.get('/api/admin/stats', auth, adminAuth, function(req, res) {
  ...
  const totalMessages = dbGet('SELECT COUNT(*) as count FROM messages');
  const totalFiles = dbGet('SELECT COUNT(*) as count FROM files');
  res.json({
...
app.get('/api/admin/users', auth, adminAuth, function(req, res) {
  const users = dbAll("SELECT u.id, u.username, u.email, u.is_temp, CASE WHEN fa.user_id IS NOT NULL THEN 1 ELSE 0 END as has_file_access FROM users u LEFT JOIN file_access fa ON u.id = fa.user_id WHERE u.id != ? ORDER BY u.id", [req.userId]);
  res.json({ users: users });
});

app.post('/api/admin/grant-file-access/:userId', auth, adminAuth, function(req, res) {
  const userId = parseInt(req.params.userId);
  const existing = dbGet('SELECT * FROM file_access WHERE user_id=?', [userId]);
  if (!existing) {
...
app.post('/api/avatars/:id/activate', auth, function(req, res) {
  const avatar = dbGet("SELECT * FROM avatars WHERE id=? AND user_id=?", [req.params.id, req.userId]);
...
app.get('/api/check-file-access', auth, function(req, res) {
  ...
  const access = dbGet('SELECT * FROM file_access WHERE user_id=?', [req.userId]);
...
app.post('/api/friends/request/:uid', auth, function(req, res) {
  ...
  const existing = dbGet("SELECT * FROM friends WHERE (from_user=? AND to_user=?) OR (from_user=? AND to_user=?)", [req.userId, toId, toId, req.userId]);
  if (existing) {
...
app.post('/api/friends/accept/:uid', auth, function(req, res) {
  const r = dbGet("SELECT * FROM friends WHERE from_user=? AND to_user=? AND status='pending'", [req.params.uid, req.userId]);
...
app.get('/api/friends', auth, function(req, res) {
  const friends = dbAll("SELECT u.id, u.username, u.avatar FROM friends f JOIN users u ON u.id = CASE WHEN f.from_user=? THEN f.to_user ELSE f.from_user END WHERE (f.from_user=? OR f.to_user=?) AND f.status='accepted'", [req.userId, req.userId, req.userId]);
  const seen = {}, unique = [];
...
app.post('/api/messages/:fid', auth, upload.single('file'), async function(req, res) {
  try {
    ...
    const friend = dbGet("SELECT * FROM friends WHERE ((from_user=? AND to_user=?) OR (from_user=? AND to_user=?)) AND status='accepted'", [req.userId, fid, fid, req.userId]);
...
app.post('/api/messages/:id/react', auth, function(req, res) {
  ...
  const userReactions = dbAll("SELECT * FROM reactions WHERE message_id=? AND user_id=?", [req.params.id, req.userId]);
...
app.post('/api/messages/:id/pin/shared', auth, function(req, res) {
  ...
  const existing = dbGet("SELECT * FROM shared_pins WHERE message_id=? AND pinned_by=?", [req.params.id, req.userId]);
...
app.post('/api/messages/:id/pin/private', auth, function(req, res) {
  ...
  const u1 = Math.min(msg.sender_id, msg.receiver_id);
  const u2 = Math.max(msg.sender_id, msg.receiver_id);
  const existing = dbGet("SELECT * FROM private_pins WHERE message_id=? AND pinned_by=?", [req.params.id, req.userId]);
  if (existing) return res.status(400).json({ error: 'Уже закреплено вами' });
...
app.get('/api/pinned/shared/:fid', auth, function(req, res) {
  ...
  const pinned = dbAll("SELECT sp.*, m.message_text, m.file_name, m.file_type, m.file_path, m.sender_id, m.created_at as msg_created, s.username, s.avatar FROM shared_pins sp JOIN messages m ON sp.message_id=m.id JOIN users s ON m.sender_id=s.id WHERE sp.chat_user1=? AND sp.chat_user2=? ORDER BY sp.created_at DESC", [u1, u2]);
...
app.get('/api/users', auth, function(req, res) {
  const users = dbAll('SELECT id, username, email, avatar, is_temp, is_online, last_seen, is_typing FROM users WHERE id!=?', [req.userId]);
  res.json({ users: users });
});
